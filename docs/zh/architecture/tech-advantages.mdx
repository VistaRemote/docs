...
| **会不会被厂商绑架？** | **开源可审计** + **私有化部署**（MySQL、MinIO、内网 AI） |
...
- 录像、审计、AI 摘要均可落在 **企业 VPC**（MySQL + MinIO + Redis + 可选 Ollama）。
...
| 组件 | 角色 | 为何省心 |
| :--- | :--- | :--- |
| **MySQL 8** | 业务库 | 国内 DBA 最熟，云 RDS 现成 |
...
| 运维组件 | 不透明 | 常膨胀 | **MySQL + Redis + Compose** |
